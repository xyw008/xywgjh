...
// 低位补0
+ (NSString *)fillZeroWithStr:(NSString *)str
{
    if (str.length % 2 != 0)
    {
        NSMutableString *resultStr = [NSMutableString stringWithString:str];
        [resultStr insertString:@"0" atIndex:0];
        return resultStr;
    }
    return str;
}
...
+ (NSString *)decimalToHex:(NSUInteger)tmpid{
    ...
    return [self fillZeroWithStr:str];
}
/**
 *  十六进制 -> 二进制
 */
+ (NSString *)hexToBinary:(NSString *)hex{
    NSString *hexStr = [self fillZeroWithStr:hex];
    NSMutableString *str = [NSMutableString stringWithString:@""];
    NSUInteger count = hexStr.length;
    for (NSInteger index = 0; index < count; index++) {
        NSString *appendStr = [[self bitHexDic] objectForKey:[[hexStr substringWithRange:(NSRange){index, 1}] uppercaseString]];
        /*
        if(index == 0){
            //过滤前面的0
            appendStr = [NSString stringWithFormat:@"%ld", [appendStr integerValue]];
