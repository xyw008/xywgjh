...
{
    _margin = margin;
    _axisWidth = self.frame.size.width - _margin - 10;
    _axisHeight = self.frame.size.height - 2 * _margin;
}
...
- (UILabel*)createLabelForValue: (NSUInteger)index
{
    ...
    UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(0, p.y + 2, width + 2, 14)];
...
- (CGPoint)getFillPointForIndex:(NSUInteger)idx withScale:(CGFloat)scale
{
    if(idx >= _data.count)
        return CGPointZero;
    
    // Compute the point position in the view from the data with a set scale value
    CGFloat value = [_data[idx] floatValue] - 32;
    return CGPointMake(_margin + idx * (_axisWidth / (_data.count - 1)), _axisHeight + _margin - value * scale);
}
...
- (UIBezierPath*)getLinePath:(float)scale withSmoothing:(BOOL)smoothed close:(BOOL)closed
{
    ...
    if(closed) {
        ...
        [path addLineToPoint:[self getFillPointForIndex:_data.count - 1 withScale:scale]];
        [path addLineToPoint:[self getFillPointForIndex:_data.count - 1 withScale:0]];
        [path addLineToPoint:[self getFillPointForIndex:0 withScale:0]];
        [path addLineToPoint:[self getFillPointForIndex:0 withScale:scale]];
